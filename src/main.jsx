  ...
  return (
    <div style={{ backgroundColor: '#0d0e12', minHeight: '100vh', padding: '30px', color: '#f1f3f9', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '25px' }}>
        <button onClick={() => setActiveSport('americanfootball_nfl')} style={{ backgroundColor: activeSport === 'americanfootball_nfl' ? '#00b37e' : '#202024', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>🏈 NFL</button>
        <button onClick={() => setActiveSport('americanfootball_ncaaf')} style={{ backgroundColor: activeSport === 'americanfootball_ncaaf' ? '#00b37e' : '#202024', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>🎓 CFB</button>
      </div>

      <div style={{ backgroundColor: '#141722', borderRadius: '12px', padding: '20px', border: '1px solid #1e293b' }}>
        {prizepicksData.length > 0 ? (
          prizepicksData.slice(0, 50).map((player, idx) => {
            ...
            const bookLine = matches[0]?.line || "Scanning...";
            ...
            return (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #1e293b' }}>
                <div>
                  ...
                  <span style={{ marginLeft: '10px', color: '#64748b', fontSize: '12px' }}>{stat.toUpperCase()}</span>
                </div>
          ...
          <div style={{ color: '#64748b', textAlign: 'center' }}>Connecting to data streams... Please refresh in 5 seconds.</div>
        )}
      </div>
    </div>
  );
